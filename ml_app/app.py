...
def get_engine():
    try:
        ...
        db = os.environ.get("SCOUTS_DW_DATABASE", "scout_DW").strip()
        server = os.environ.get("SCOUTS_ODBC_SERVER", "localhost,1433").strip()
...
def load_data():
    ...
    # Objectives 1-2: ALWAYS from Excel (long history), never blocked by DW.
    try:
        primary_members = _project_file('Scout_Evolution_Saison.xlsx')
        fallback_members = _project_file('members_data.xlsx')
        if os.path.exists(primary_members):
            members = pd.read_excel(primary_members)
        elif os.path.exists(fallback_members):
            members = pd.read_excel(fallback_members)
    except Exception as ex:
        print(f"WARN: Could not read membership Excel: {ex}")
    ...
    # Objectives 3+: try scout_DW; fallback gracefully if unreachable.
    engine = get_engine()
    if engine is None:
        print("INFO: Hybrid mode active (Excel for Obj1-2, DW unavailable for Obj3+).")
        return members, weather, activities, avg_cost_day, sponsors, "Hybrid"
    ...
    camps = pull("Fact_Camp", "SELECT Participants_count, Duration_Days, Total, Saison FROM dbo.Fact_Camp")
    if not camps.empty:
        camps = camps.copy()
    ...
    sponsors = pull(
        "Fact_Sponsors",
        "SELECT u.unit_code, fs.promised_amount_TND, fs.Received_amount_TND "
        "FROM dbo.Fact_Sponsors fs JOIN dbo.dim_unite u ON fs.Unit_FK = u.unit_id",
    )
    weather = pull(
        "Fact_Weather_Events",
        "SELECT temp_max_mean, Total_Rainfall AS rain_total, Max_win_speed AS wind_max FROM dbo.Fact_Weather_Events",
    )
    ...
    print("INFO: Hybrid mode active (Excel Obj1-2 + DW Obj3+).")
    return members, weather, activities, avg_cost_day, sponsors, "Hybrid"
...
if __name__ == '__main__':
    ...
    try:
        ...
        if eng is not None:
            ...
            print("WARNING: Weather model skipped (DW unavailable).")
    except Exception as e:
    ...
    print(f"INFO: Flask listening on http://127.0.0.1:{run_port}/")
    try:
